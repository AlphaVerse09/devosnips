...
import { ClipboardCopy, Edit3, Trash2, FileQuestion, FolderKanban, Eye } from "lucide-react";
...
export function SnippetItem({ snippet, onEdit, onDelete }: SnippetItemProps) {
  ...
  const CategoryIcon = () => {
    if (snippet.category === 'Other') {
      return <FileQuestion className="h-4 w-4" />;
    }
    return <DevIcon category={snippet.category} className="h-4 w-4" />;
  };
